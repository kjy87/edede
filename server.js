...
const youtube = new YouTube("AIzaSyDzF0_rr5XePx2qxnBliwHS3xmH-pNu-yA"); //تعديل اساسي سوي اي بي اي جديد
const hastebins = require("hastebin-gen");
const getYoutubeID = require("get-youtube-id");
const yt_api_key = "AIzaSyDzF0_rr5XePx2qxnBliwHS3xmH-pNu-yA"; ///تعديل اساسي سوي اي بي اي جديد
...
client.on("message", message => {
  if (message.content === prefix + "bot") {
    const bot = new Discord.RichEmbed()
      ...
      .addField("**Bot Owner** :  ", `» <@479820186690650112>`, true) // تعديل اساسي غير الايدي لايدي حسابك
      .setImage("")
...
client.on("message", message => {
  if (!message.channel.guild) return;
  let room = message.content.split(" loogs").slice(1);
...
client.on("message", message => {
  if (message.content === "السلام عليكم") {
    ...
    message.channel.sendFile("اغاني🤭♥");
  }
});
...
client.on("message", message => {
  let args = message.content.split(" 20").slice(1);
